    ...
    <div className="min-h-screen bg-background text-white" dir="rtl">
      ...
      {assignRound !== null && (
        <div className="fixed inset-0 z-40 bg-black/70 flex items-center justify-center p-4">
          <div className="bg-gray-900 rounded-2xl p-5 w-full max-w-md max-h-[80vh] flex flex-col">
            <h3 className="ar font-bold text-white mb-3">تحديد أسئلة الجولة {assignRound}</h3>
            <p className="ar text-xs text-gray-500 mb-3">اختر الأسئلة المخصصة لهذه الجولة (اتركها فارغة للاختيار التلقائي)</p>
            <div className="flex-1 overflow-y-auto space-y-1 mb-4">
              {deckQs.map((q, i) => (
                <label key={i} className="flex items-start gap-3 p-2 rounded-lg hover:bg-gray-800 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={selectedQIdxs.includes(i)}
            ...
            <div className="flex gap-3">
              <button onClick={() => setAssignRound(null)} className="flex-1 py-2.5 rounded-xl bg-gray-800 text-gray-300 text-sm ar">إلغاء</button>
              <button onClick={saveAssignment} className="flex-1 py-2.5 rounded-xl bg-primary text-background font-bold text-sm ar">حفظ</button>
            </div>
          </div>
        </div>
      )}
